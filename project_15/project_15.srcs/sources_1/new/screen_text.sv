module pingpong_screen_text(
    ...
    logic [20:0] xText, yText; //Text内坐标
    ...
    always @(*)
        case (xText[7:3])    // xText[2:0]为1个字
            ...
            13:   charAddr = 7'h73;//'s'
            14:   charAddr = 7'h3a;//':'
            15:   charAddr = 'h30 + ball_count; // 十位数
        endcase
    ...
    always_comb
        case(xText2[7:3])
            ...
            4'h9: charAddr2 = 7'h21; // !
            default : charAddr2 = 32;
        endcase

    assign rowAddr2 = yText2[3:0];
    assign txtAddr2 = {charAddr2, rowAddr2};

    ASCII_ROM A2(.a(txtAddr2), .spo(textData2));

    assign One_bit2 = textData2[xText2[2:0]];

    logic textOn2;
    assign textOn2 = ((pix_x > H_SYNC_START+C3)     &&
                     (pix_x < H_SYNC_START+C3+8*8) && //8字符*8bits
                     (pix_y > V_SYNC_START+R3)     && 
                     (pix_y < V_SYNC_START+R3+16)) && //16bits高
                     One_bit2 ;  // 像素=1？
